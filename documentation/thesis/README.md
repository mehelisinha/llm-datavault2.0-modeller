...
| [`experiment-3-cross-domain-control.md`](./experiment-3-cross-domain-control.md) | **Experiment 3 — CIM cross-domain control.** Findings: cross-domain examples are **inert on CIM** (but the unfiltered live corpus *hurts* ServiceNow — findings §2.8 — so "safe" is schema-dependent, not universal); the naming effect is **invisible on CIM because its tables are already concept-named** → the effect's scope condition is a *table-vs-concept naming gap* (e.g. ServiceNow prefixes). Consolidated 1–3 map of where feedback helps, is inert, and is a liability. |
