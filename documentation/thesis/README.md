...
| [`experiment-6-drift.md`](./experiment-6-drift.md) | **Experiment 6 — Use Case B drift (RQ2).** Code built + tested (`drift/` package: additive/cosmetic/breaking taxonomy, rule-only baseline, LLM classifier, cost-weighted scoring). **H2a** (deterministic recall) demonstrated on a synthetic pair. **H2b synthetic PoC:** AI accuracy 0.90 > rule 0.80, **but** AI is *less safe* (breaking-recall 0.75 vs 1.00; cost 5 vs 2) — it missed an orphaned-table breaking change. Design implication: **hybrid** (AI subtlety + rule safety-floor). Real 6b/6c await the author's drifted Databricks dataset. |
