...
from dbt_builder.src.dv_components.components.base import DVBaseStagingComponent
from dbt_builder.src.dv_components.pydantic_model.sql.staging import StagingModel


class StagingComponent(DVBaseStagingComponent):
    """Data Vault Hub component."""

    def __init__(self, model: StagingModel, logger: Logger):
        super().__init__(model=model, logger=logger)

        if not isinstance(model, StagingModel):
            raise ValueError(f"Expected StagingModel, got {type(model)}")
        self.stg_model: StagingModel = model
...
"""
    ...
    # ===========""")
    stg = StagingComponent(model=model, logger=default_logger)
    sql = stg.generate_sql_str()
