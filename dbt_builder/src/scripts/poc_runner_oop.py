...
class Config:
    ...
    DEFAULT_METADATA_PATH: ClassVar[Path] = (
        SRC_DIR / "metadata" / "iec_cim_metadata.yaml"
    )
...
class Validator:
    ...
    def validate(self) -> None:
        ...
        for table in self.TABLES:
            full_table = f"{self.catalog}.{self.raw_vault_schema}.{table}"
...
class PocRunner:
    def __init__(
        ...
        self.validator = Validator(
            catalog=self.catalog, raw_vault_schema=self.raw_vault_schema
        )
