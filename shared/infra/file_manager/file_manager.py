...
from shared.infra.file_manager.file_handler_factory import FileHandlerFactory
...
class FileManager:
    ...
    @staticmethod
    def write(file_path: str | Path, content: Any, logger: Logger= default_logger, overwrite: bool = False):
        try:
            path = Path(file_path)
            handler = FileHandlerFactory.get(path.suffix.lstrip("."))
