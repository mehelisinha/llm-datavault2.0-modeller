...
from src.dv_components.dv_model_factory import DVComponentFactory
...
class DVComponentManager:
    ...
    @classmethod
    def generate_sql(cls, model: DVComponentModel, source_models: list[str], logger:Logger = default_logger) -> str:
        component = DVComponentFactory.create(model=model, source_models=source_models,logger=logger)
        return component.generate_sql()

    @classmethod
    def write_sql_file(cls, model: DVComponentModel, source_models: list[str], project_path: str, logger:Logger = default_logger) -> str:
